...
final class OrderController extends AbstractController
{
    ...
    public function index(
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($order->isPaidOnDelivery()) {
                // Pay on delivery: send confirmation email, clear cart
    ...
    {
        return $this->json(['status' => 200, 'content' => $city->getShippingCost()]);
    }
    ...
    public function getAllOrder(
        ...
        if ($type === 'is-completed') {
            ...
            $criteria = ['isPaymentCompleted' => true, 'paidOnDelivery' => false, 'isCompleted' => false];
        } elseif ($type === 'pay-online-completed') {
            $criteria = ['isPaymentCompleted' => true, 'isCompleted' => true];
        }

        $orders = $paginator->paginate(
            $orderRepository->findBy($criteria, ['id' => 'DESC']),
    ...
    {
        $html = $this->renderView('email/order_email.html.twig', ['order' => $order]);
